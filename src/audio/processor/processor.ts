import createContext, { IAudioContextOptions } from "../context/createContext"
import { IN_DEV } from "../../utils/env"

import { ISamples } from "../../redux/store/sample/interfaces"
import { ICell, ITrack, ITracks } from "../../redux/store/session/interfaces"
import {
    IInstrument,
    IInstruments
...
class AudioProcessor {
    ...
    setMasterGainNodeValue = (gain: number) => {
        ...
        console.debug(
            "[AudioProcessor] master gain set to %d at %d s",
            gain,
            time
        )
    }
    ...
    setTrackGainNodeValue = (trackID: string, gain: number) => {
        ...
        if (gainNode instanceof GainNode) {
            ...
            console.debug(
                "[AudioProcessor] track gain set to %d at %d s",
                gain,
                time
            )
        }
    ...
    setTrackGainNode = (trackID: string, gain: number) => {
        IN_DEV &&
            console.groupCollapsed(
                "[AudioProcessor] setTrackGainNode %s",
                trackID
            )
        ...
        IN_DEV &&
            console.debug("[AudioProcessor] gain node --> master gain node")

        const time = this.ctx.currentTime
        gainNode.gain.setValueAtTime(gain, time)
        IN_DEV &&
            console.debug(
                "[AudioProcessor] gain node set to %d at %d",
                gain,
                time
            )
    ...
    playSample(
        ...
        note: number | null,
        trackID: string,
        tracks: { [p: string]: ITrack },
        instruments: { [p: string]: IInstrument },
        matrix: { [p: string]: ICell[] }
    ) {
        ...
        if (!mapping["M" + midi]) {
            IN_DEV &&
                console.debug("No mapping found for note %d - aborting", midi)
            return
        }
        ...
        IN_DEV &&
            console.groupCollapsed("[AudioProcessor] playSample %s", sampleID)
        ...
        IN_DEV &&
            console.debug("[AudioProcessor] detune value set to %d", detune)
    ...
    scheduleNoteForTrack = (
        ...
        console.assert(
            audioNodes instanceof Map,
            "ITrack audio node map entry for track %s should be defined",
            trackID
        )
